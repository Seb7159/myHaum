...
if($password=="abc"){
	...
	while($row=mysql_fetch_array($result)){
		...
		if(!($ay==$ny && $am==$nm && $ad==$nd)){
			...
			$subject = "myHaum Gas overflow";

			$message = "
			<html>
			<head>
			<title>myHaum Gas overflow</title> 
			</head>
			<body>
			<h1><center>Warning!</center></h1>
			<h3>The gas sensor has overflown right now, on ".date("d-M-Y H:i:s").". <br><br>Please take action immediately!</h3>
